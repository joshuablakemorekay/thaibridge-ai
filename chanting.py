...
_CONTENTS_ROMAN = {
    ...
    # ── Thai, in Paiboon — written by hand ──
    'ทำวัตรเช้า':
        'tam-wát cháao',
    'คำบูชาพระรัตนตรัย':
    ...
    # ── Pali, in IAST — generated by scripts/romanise_contents.py ──
    'ระตะนัตตะยัปปะณามะคาถา':
    ...
    # ── Pali, in IAST — but this one is HAND-WRITTEN, not generated ──
    # The book spells this title in Thai orthography rather than the phonetic
    # style it uses for Pali everywhere else, so the script refuses it (ยว and
    # ตร are not Pali clusters, and a Pali word cannot end in ร) and would
    # produce Sekhiyvatr. The word is Pali all the same, and the chant's own
    # title_pali has read it as Sekhiyavatta since long before this table.
    # `--check` reports it as a disagreement; this comment is the answer.
    'เสขิยวัตร':
        'Sekhiyavatta',
