...
- 🔊 **A pronunciation system built for each reader** *(live)* — the English side of Monk Mode carries native audio on all 140 items, a plain respelling with the stressed syllable in capitals, a tip written in Thai, and IPA behind a toggle for anyone who wants it. **New: a choice of British or American English** — flip the accent and the word (aeroplane/airplane), the respelling (MAW-ning/MOR-ning) and the voice all switch together, with 185 audio files per accent. The audio is generated once and committed as ordinary files, so the live site needs no speech service, no API key and no per-play cost
- 🗣️ **Teacher-sourced drills & Dhamma readings** *(new)* — real material from my Thai teacher became two lessons: L/R tongue twisters ("rice" vs "lice") with per-line audio, and three Ajahn Sumedho readings with whole-passage audio in either accent — source credited, free-distribution honoured
