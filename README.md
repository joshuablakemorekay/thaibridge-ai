...
> **Honest status:** this is a learning project, not a finished product — but it's now **live on the internet** at 👉 **[thaibridge-ai.smoald.com](https://thaibridge-ai.smoald.com)** (custom domain, hosted free on Render). The full build story (including the mistakes) is in [`JOURNAL.md`](./JOURNAL.md).
...
- **10 Jun 2026 — Made it work on phones (responsive UI).** The app looked great on a laptop but broke on my iPhone — sidebar pages squashed text into a one-word-per-line strip and the AI Tutor page spilled off the screen. I fixed it as **one responsive site** (not a separate mobile app): the layouts now collapse to a single column on phones, and the headings, logo and chat header **scale smoothly with screen width** (`clamp()`), so the same link works from a 320px phone up to a desktop. I tested it across seven screen widths and confirmed nothing overflows, with the desktop look unchanged. **Lesson:** CSS media queries add no "weight" — a more specific desktop rule beats a mobile one regardless of screen size, which is exactly why the layout wouldn't collapse until I matched their specificity.
- **9 Jun 2026 — Live on the internet (with a working AI link).** I deployed ThaiBridge to Render so anyone can visit it at **[thaibridge-ai.smoald.com](https://thaibridge-ai.smoald.com)**, and proved the AI tutor responds live by sending it a real message. I made the public demo **cost-safe** — it runs the cheap **Haiku** model and my Anthropic spend is capped — and added a free uptime pinger so the link loads instantly. **Lesson:** when the live app flickered with errors, the server logs showed it was just the free tier waking from sleep, not the crash I'd feared — checking the logs beat guessing from outside.
