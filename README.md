...
- **A rule is only as good as the fact underneath it.** My alphabet chart used emoji rather than pictures because 44 images at once was the page weight that once crashed its renderer. That was a rule about *photographs* — once the pictures became vector drawings (30 KB for all 44, less than one photo) the reason had quietly expired, and two letters were still showing a drum and a trophy because Unicode has no cymbals or offering-tray character. **So I wrote the expiry condition into the code itself: if these ever become photographs, the chart goes back to emoji.** A rule that outlives its reason is worse than no rule, because nobody thinks to re-check it.
