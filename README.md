...
- 🔊 **Tap-to-hear audio on the learning pages** *(new)* — a reusable system that puts a 🔊 button next to Thai words so you hear a native voice; live on the Vowels and Sentences pages, with a shared player so any page can use it. The audio is generated once and committed as ordinary files — no speech service, no API key, no per-play cost
...
- **Roll audio out to the rest of the learning track** — nine pages still have no tap-to-hear audio (Tones, Paiboon, Grammar, Formality, Gender Guide, Vocabulary, Lessons, Tour Guide, Business Thai). The reusable system is built, so each page is a small, repeatable job — though some store their Thai inline and will need it pulled into data first.
- **Get the Thai reviewed by my teacher** — a generated review sheet (`docs/review-2026-07-20-monk-english.md`, 152 items across 10 lessons) is ready to send, covering every AI-drafted pronunciation tip, sentence translation and word-choice note. The script that builds it diffs against an earlier commit (compares versions to see what changed), so future rounds only ask about material she hasn't already corrected. Newly added and also needing her ear: the five Phase-2 conversations and everyday phrases, especially the temple/alms-round Thai. Still outstanding too: the two Pāli terms spoken by an English voice, and giving the pronunciation guide page its own accent section (its British-only footnote is out of date).
