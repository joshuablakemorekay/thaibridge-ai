...
- 🔤 **[Thai Alphabet](https://thaibridge-ai.onrender.com/alphabet)** *(free)* — all 44 consonants with a tap-to-hear chart, flashcards and a 44-question quiz, each letter recorded in Thai. **New: every letter now carries a picture of the word it's named after** — ก is "gɔɔ gài, chicken", so ก shows a chicken — on the chart, the flashcards and a new picture round in the quiz. Passing it (35/44) unlocks the rest of the site
- 🔤 **Vocabulary lessons**, written so complete beginners can read straight away
- ✍️ **[Read & Write Thai Script](https://thaibridge-ai.onrender.com/read_write)** *(new · Basic tier)* — the bridge from knowing the letters to using them: a reading trainer that teaches how a Thai vowel can sit *before, above, below or around* its consonant (you always say the consonant first), a trace-over-the-guide canvas for forming all 42 written letters by hand, and a short **Jataka tale** you read one tapped word at a time — dharma stories as reading material
...
- **Add real photographs for the 44 letters.** The page picks up a picture by filename with no code change, so it's a drop-in job — and it would let the twelve letters with only-approximate emoji rejoin the quiz's picture round.
